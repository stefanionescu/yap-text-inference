...
echo "[build:engine] Engine build..."
...
TRTLLM_MAX_INPUT_LEN="${TRTLLM_MAX_INPUT_LEN:-60}"
...
if [[ -d ${ENGINE_OUTPUT_DIR} && ${FORCE_REBUILD:-false} != true ]]; then
  echo "[build:engine] Reusing existing engine at ${ENGINE_OUTPUT_DIR}"
else
  if [[ ${PRECISION_MODE} == "base" ]]; then
    echo "[build:engine] Building TensorRT engine (base ${TRTLLM_DTYPE})..."
  else
    echo "[build:engine] Building TensorRT INT4-AWQ engine..."
  fi
  build_cmd=(
    trtllm-build
  ...
  echo "[build:engine] Running: ${build_cmd[*]}"
  "${build_cmd[@]}"

  echo "[build:engine] Recording build command and metadata..."
...
PY
  ...
  if [[ ${PRECISION_MODE} == "base" ]]; then
    # Load resolved qformat from quantization step
    RESOLVED_QFORMAT="${TRTLLM_DTYPE}"
    RESOLVED_KV_DTYPE="none"
    if [[ -f .run/quantize_result.env ]]; then
      # shellcheck disable=SC1091
      source .run/quantize_result.env
    fi
    # Build metadata based on quantization format
    if [[ "${RESOLVED_QFORMAT}" == "full_prec" ]]; then
      quantization_meta="  \"quantization\": {\"weights\": \"none\", \"kv_cache\": \"none\", \"dtype\": \"${TRTLLM_DTYPE}\"},"
    elif [[ "${RESOLVED_QFORMAT}" == "fp8" ]]; then
      quantization_meta="  \"quantization\": {\"weights\": \"fp8\", \"kv_cache\": \"${RESOLVED_KV_DTYPE}\"},"
    else
      quantization_meta="  \"quantization\": {\"weights\": \"${RESOLVED_QFORMAT}\", \"kv_cache\": \"${RESOLVED_KV_DTYPE}\"},"
    fi
  else
...
META
  ...
  if [[ -f "${hf_model_dir}/generation_config.json" ]]; then
    ...
    echo "[build:engine] Copied generation_config.json to engine directory"
  fi
fi
