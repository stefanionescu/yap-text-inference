...
echo "[build:remote] Checking remote deploy configuration..."

SKIP_QUANTIZATION="${SKIP_QUANTIZATION:-false}"

if [ -z "${HF_DEPLOY_REPO_ID:-}" ]; then
  echo "[build:remote] HF_DEPLOY_REPO_ID not set → no remote deploy"
  exit 0
fi

export HF_HUB_ENABLE_HF_TRANSFER=1

echo "[build:remote] Querying Hugging Face repo: ${HF_DEPLOY_REPO_ID}"
...
if selected:
    path=snapshot_download(repo_id=repo_id, local_dir=workdir,
...
case "$mode" in
  engines)
    ...
    if _engine_env_compatible "$eng_dir"; then
      ...
      echo "[build:remote] Using prebuilt engine: $TRTLLM_ENGINE_DIR"
      mkdir -p .run
      echo "REMOTE_RESULT=10" >.run/remote_result.env
    else
      echo "[build:remote] Downloaded engines incompatible; will try checkpoints if available"
  ...
  checkpoints)
    ...
    if [ -n "$ckpt_dir" ]; then
      ...
      echo "[build:remote] Using downloaded checkpoint: $CHECKPOINT_DIR"
      mkdir -p .run
    ...
    ;;
  error)
    echo "[build:remote] WARNING: HF repo query failed; continuing with local path" >&2
    ;;
  none | *)
    echo "[build:remote] No usable remote artifacts; continuing with local quantization/build"
    ;;
esac

echo "[build:remote] OK"
