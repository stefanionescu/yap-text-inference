...
# Validate HF_DEPLOY_REPO_ID contains TRT-LLM and AWQ indicators (case insensitive)
...
if [[ -n ${HF_DEPLOY_REPO_ID:-} ]]; then
  ...
  # Accept "trt", "trtllm", "trt-llm", or "tensorrt" for TRT indicator
  if [[ ! $_repo_lower =~ (trt|tensorrt) ]] || [[ ! $_repo_lower =~ awq ]]; then
    echo "ERROR: AWQ Docker image requires a pre-quantized TRT-LLM AWQ engine repo." >&2
    echo "       HF_DEPLOY_REPO_ID must contain TRT indicator (trt/trtllm/tensorrt) and 'awq'." >&2
  ...
  if [[ $mode == "engines" ]]; then
    ...
    if [[ -f "$CHECKPOINT_DIR/config.json" ]] && ls "$CHECKPOINT_DIR"/rank*.safetensors >/dev/null 2>&1; then
      ...
      trtllm-build \
        ...
        --max_input_len "${TRTLLM_MAX_INPUT_LEN:-60}" \
        --max_seq_len "$((${TRTLLM_MAX_INPUT_LEN:-60} + ${TRTLLM_MAX_OUTPUT_LEN:-1162}))" \
...
echo "[server] Tailing ${LOG_FILE} (Ctrl-C to detach; server keeps running)"
