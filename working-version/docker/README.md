...
- PyTorch 2.9.0 + torchvision 0.24.0 (cu130 wheels)
...
- Runtime scripts: `start_server.sh` and `environment.sh`
...
Pulls engines or checkpoints from `yapwithai/orpheus-3b-tts-trt-awq` by default. If engines match your GPU/driver, the container skips build; if only checkpoints exist, it builds the engine locally.

```bash
# Uses default HF_DEPLOY_REPO_ID=yapwithai/orpheus-3b-tts-trt-awq
docker run --gpus all --rm -p 8000:8000 \
  -e HF_TOKEN=$HF_TOKEN \
  -e ORPHEUS_API_KEY=$ORPHEUS_API_KEY \
  IMAGE:TAG

# Or specify a custom repo/engine label
docker run --gpus all --rm -p 8000:8000 \
  -e HF_TOKEN=$HF_TOKEN \
  -e ORPHEUS_API_KEY=$ORPHEUS_API_KEY \
...
- `HF_DEPLOY_REPO_ID` (default: `yapwithai/orpheus-3b-tts-trt-awq`): HF repo to pull from
