...
echo ""
echo "[server] TTS Server Startup"
...
if [ ! -d "$VENV_DIR" ]; then
  ...
  echo "Run scripts/steps/01-install-trt.sh first" >&2
  exit 1
fi
...
if [ ! -f "$TRTLLM_ENGINE_DIR/rank0.engine" ]; then
  if [ -d "$TRTLLM_ENGINE_DIR" ] && [ -f "$TRTLLM_ENGINE_DIR/../build_metadata.json" ]; then
    ...
    echo "Run scripts/steps/02-build.sh first or configure HF_DEPLOY_REPO_ID to pull prebuilt engines." >&2
    exit 1
  fi
fi
...
bash "scripts/runtime/warmup.sh"
