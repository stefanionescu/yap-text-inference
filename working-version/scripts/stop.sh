...
_clear_runtime_state() {
  ...
  # Preserve build_config.env and last_config.env so restart.sh knows the last successful build
  while IFS= read -r -d '' entry; do
    local name
    name=$(basename "$entry")
    case "$name" in
      build_config.env | last_config.env)
        continue
        ;;
      *)
  ...
  done < <(find "$run_dir" -mindepth 1 -maxdepth 1 -print0 2>/dev/null || true)
}
