"""History parsing and user utterance normalization helpers for start messages."""
...
def resolve_history(
    ...
    if _history_turn_count(state) > 0:
        return session_handler._history.get_chat_messages(state)
    ...
    if history_config.deploy_chat and history_config.deploy_tool:
        ...
        session_handler._history.set_mode_histories(state, chat_messages=chat_messages, tool_turns=tool_turns)
    elif history_config.deploy_tool and not history_config.deploy_chat:
        tool_turns = parse_history_for_tool(history_messages)
        session_handler._history.set_mode_histories(state, tool_turns=tool_turns)
    else:
        chat_messages = parse_history_for_chat(history_messages)
        session_handler._history.set_mode_histories(state, chat_messages=chat_messages)
    return session_handler._history.get_chat_messages(state)
