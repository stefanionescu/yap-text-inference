...
async def _create_llm_instance(kwargs: dict[str, Any]) -> Any:
    ...
    from src.scripts.filters.trt import configure_trt_logger  # noqa: PLC0415
...
async def create_trt_engine() -> TRTEngine:
    ...
    if not engine_dir or not os.path.isdir(engine_dir):
        raise RuntimeError(f"TRT_ENGINE_DIR must point to a valid TensorRT-LLM engine directory. Got: {engine_dir!r}")
