...
_LICENSE_FILE_NAMES = ("license", "copying", "notice")
...
def _parse_requirement_name(raw_value: str) -> str | None:
    ...
    if value.startswith(("-e", "--editable")):
        if "#egg=" in value:
            return _normalize_name(value.split("#egg=", 1)[1])
        return None
    if " @ " in value:
        return _normalize_name(value.split(" @ ", 1)[0])
...
def _license_from_metadata(dist: metadata.Distribution) -> str:
    known = _first_known_license([dist.metadata.get("License-Expression", "")])
    if known:
        return known
...
def _config_strings(config_doc: dict[str, object], key: str) -> list[str]:
    return [str(value).strip() for value in config_doc.get(key, []) if isinstance(value, str) and str(value).strip()]
...
def _load_policy() -> tuple[set[str], list[str], set[str], dict[str, str]]:
    config_doc = load_config_doc("security", "licenses.toml")
