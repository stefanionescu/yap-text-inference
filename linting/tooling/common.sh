...
TOOLING_CONFIG_DIR="${REPO_ROOT}/linting/config/tooling"

# source_tooling_config - Load a repo-local linting-tool config file by name.
source_tooling_config() {
  local config_name="$1"
  local config_path="${TOOLING_CONFIG_DIR}/${config_name}"
  ...
  if [[ ! -f ${config_path} ]]; then
    echo "error: missing linting tool config: ${config_path}" >&2
    exit 1
  fi

  # shellcheck disable=SC1090  # lint:justify -- reason: linting tool wrappers intentionally source repo-local config files by path -- ticket: N/A
  source "${config_path}"
}
...
require_download_command() {
  if ! command -v curl >/dev/null 2>&1; then
    echo "error: curl is required for fallback lint tool installation" >&2
    exit 1
  fi
}

# repo_tool_cache_dir - Return the shared cache directory for downloaded lint tools.
repo_tool_cache_dir() {
  local cache_dir="${REPO_ROOT}/${TOOLING_CACHE_RELATIVE_DIR}"
  mkdir -p "${cache_dir}"
  echo "${cache_dir}"
}

# repo_tool_bin_dir - Return the repo-local directory for installed lint tool symlinks.
repo_tool_bin_dir() {
  local bin_dir
  bin_dir="$(repo_tool_cache_dir)/bin"
  mkdir -p "${bin_dir}"
  echo "${bin_dir}"
}

# repo_tool_dir - Return the repo-local directory for a named lint tool.
repo_tool_dir() {
  local tool_name="$1"
  local tool_dir
  tool_dir="$(repo_tool_cache_dir)/${tool_name}"
  mkdir -p "${tool_dir}"
  echo "${tool_dir}"
}
...
link_tool() {
  local binary_path="$1"
  local tool_name="$2"
  ln -sf "${binary_path}" "$(repo_tool_bin_dir)/${tool_name}"
}
...
# resolve_tool_command - Resolve a system, cached, or auto-installed lint tool binary.
resolve_tool_command() {
  local tool_name="$1"
  local install_target="${2:-$1}"
  local cached_binary
  cached_binary="$(repo_tool_bin_dir)/${tool_name}"
  ...
  if bash "${REPO_ROOT}/linting/tooling/install.sh" "${install_target}" >/dev/null 2>&1 && [[ -x ${cached_binary} ]]; then
    echo "${cached_binary}"
    return 0
  fi
  return 1
...
# resolve_required_tool_command - Resolve a lint tool binary or abort with a clear error.
