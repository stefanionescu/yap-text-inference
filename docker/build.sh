...
# Custom tag (optional - defaults to deploy mode if not set)
TAG="${TAG:-${DEPLOY_MODELS_VAL}}"
...
usage() {
    ...
    echo ""
    echo "Build and push Yap Text Inference Docker image"
    echo ""
    echo "The image is configured at build time with specific models."
    ...
    echo "  TAG                 - Custom image tag (default: value of DEPLOY_MODELS)"
    ...
    echo "    TAG=mistral-24b \\"
    echo "    ./build.sh"
    echo ""
    echo "  # Build tool-only image"
    echo "  DOCKER_USERNAME=myuser \\"
    echo "    DEPLOY_MODELS=tool \\"
    echo "    TOOL_MODEL=yapwithai/your-tool-model \\"
    echo "    ./build.sh"
    echo ""
