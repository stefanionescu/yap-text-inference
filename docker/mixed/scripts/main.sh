...
usage() {
  ...
  echo ""
  echo "Runtime knobs:"
  echo "  DEPLOY_MODELS=both|chat|tool|dual     (default: both if both embedded)"
  echo "  CONCURRENT_MODEL_CALL=0|1        (default 0=sequential; set to 1 for concurrent routing)"
  echo "  When DEPLOY_MODELS=dual, set DUAL_MODEL (or CHAT_MODEL) to reuse one model for chat + tool"
  echo ""
  echo "Examples:"
  echo "  docker run -d --gpus all IMAGE"
  echo "  docker run -d --gpus all -e DEPLOY_MODELS=chat IMAGE"
  echo "  docker run -d --gpus all -e CONCURRENT_MODEL_CALL=1 IMAGE  # enable concurrent routing"
  echo ""
