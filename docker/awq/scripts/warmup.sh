...
if command -v python3 >/dev/null 2>&1; then
  ...
  log_warmup "ERROR: Unable to locate python interpreter."
  exit 1
fi
...
log_warmup "Warmup: waiting for server readiness on ${SERVER_ADDR}..."
wait_for_ready
...
if [[ -z "${max_conn}" || "${max_conn}" =~ [^0-9] ]]; then
  log_warmup "WARNING: MAX_CONCURRENT_CONNECTIONS not set or invalid, defaulting to 8"
  max_conn=8
fi
if (( max_conn <= 0 )); then
  log_warmup "WARNING: MAX_CONCURRENT_CONNECTIONS is <= 0, defaulting to 8"
  max_conn=8
fi
...
if [[ "${ok}" -eq 1 ]]; then
  log_warmup "SUCCESS: warmup + bench complete."
else
  log_warmup "COMPLETE: warmup finished with failures."
fi
