...
usage() {
  ...
  echo "  DEPLOY_MODELS=both|chat|tool|dual  (default: both)"
  ...
  echo "  If DEPLOY_MODELS=dual  -> DUAL_MODEL or CHAT_MODEL required (shared model for chat + tool)"
  echo ""
  ...
  echo "  CONCURRENT_MODEL_CALL=0|1       - Default 0=sequential; set to 1 for concurrent tool/chat routing"
  echo "  (Dual deployments reuse the chat batching/token limits; tool sampling stays fixed.)"
  echo ""
  echo "Examples:"
  echo "  # Both models"
