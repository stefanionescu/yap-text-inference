...
Prompt sources:
- tests/support/prompts/base.py: Basic prompts (Anna/Mark flirty only)
  Used by: warmup, bench, connections, vision, tool, history tests

- tests/support/prompts/detailed.py: Full personality matrix
  Used by: live client persona registry and scripts that accept --persona
"""

from __future__ import annotations

from tests.support.prompts.detailed import PERSONALITIES
from tests.support.helpers.errors import PromptSelectionError
from tests.support.prompts.base import MALE_PROMPT, FEMALE_PROMPT
...
def select_chat_prompt(gender: str | None) -> str:
    ...
    if not prompt or not prompt.strip():
        raise PromptSelectionError(
            ...
            "Check that tests/support/prompts/base.py is correctly configured."
        )
...
def select_detailed_chat_prompt(gender: str | None, personality: str | None) -> str:
    ...
    This function looks up the full prompt from tests/support/prompts/detailed.py
    ...
    if persona_entry is None:
        ...
        raise PromptSelectionError(f"No prompt found for '{lookup_key}'. Available combinations: {available}")

    prompt = persona_entry.get("prompt", "")
    if not prompt or not prompt.strip():
        raise PromptSelectionError(
            f"Prompt for '{lookup_key}' is empty. Check that tests/support/prompts/detailed.py is correctly configured."
        )
...
def get_persona_info(gender: str | None, personality: str | None) -> dict[str, str]:
    ...
    if persona_entry is None:
        available = ", ".join(sorted(PERSONALITIES.keys()))
        raise PromptSelectionError(f"No persona found for '{lookup_key}'. Available combinations: {available}")
