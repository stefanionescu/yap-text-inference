...
def select_chat_prompt(gender: str | None) -> str:
    ...
    if not prompt or not prompt.strip():
        raise PromptSelectionError(
            f"Selected prompt for gender '{gender}' is empty. Check that tests/prompts/base.py is correctly configured."
        )
...
def select_detailed_chat_prompt(gender: str | None, personality: str | None) -> str:
    ...
    if not prompt or not prompt.strip():
        raise PromptSelectionError(
            f"Prompt for '{lookup_key}' is empty. Check that tests/prompts/detailed.py is correctly configured."
        )
