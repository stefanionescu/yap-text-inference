"""Persona session and variant dataclasses for personality tests."""
...
class PersonaSession:
    ...
    history: str = ""
    prompt_index: int = 0
    prompts: Sequence[str] = field(default_factory=lambda: tuple(PERSONALITY_CONVERSATION_MESSAGES))
    ...
    def next_script_prompt(self) -> str:
        ...
        if not self.has_remaining_prompts():
            raise RuntimeError("PERSONALITY_CONVERSATION_MESSAGES is empty; cannot produce user prompts.")
