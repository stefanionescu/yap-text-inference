...
case "${QUANTIZATION}" in
  fp8)
    ...
    *A100*)
    ...
    ;;
  awq)
    # 4-bit AWQ via vLLM auto-AWQ. Prefer V1 engine + FlashInfer when available.
    case "${GPU_NAME}" in
      *H100*|*L40S*|*L40*)
        ...
        ;;
      *A100*)
        # A100: Prefer V1 + FlashInfer when available, fallback to V0 + XFORMERS
        if [ "${HAS_FLASHINFER}" = "1" ]; then
          export VLLM_USE_V1=1
          export KV_DTYPE=${KV_DTYPE:-int8}
          export VLLM_ATTENTION_BACKEND=${VLLM_ATTENTION_BACKEND:-FLASHINFER}
        ...
        ;;
      *)
        # Unknown GPU: prefer FlashInfer when available
        export VLLM_USE_V1=${VLLM_USE_V1:-1}
        export KV_DTYPE=${KV_DTYPE:-auto}
        if [ "${HAS_FLASHINFER}" = "1" ]; then
          export VLLM_ATTENTION_BACKEND=${VLLM_ATTENTION_BACKEND:-FLASHINFER}
...
# AWQ quantization is now handled by scripts/quantization/awq_quantizer.sh
