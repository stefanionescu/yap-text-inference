...
vllm_install_flashinfer() {
  ...
  if [ -n "${cuda_ver:-}" ] && [ -n "${torch_ver:-}" ]; then
    ...
    log_info "Installing ${fi_pkg} (extra-index: ${fi_index})"
    if ! "${ROOT_DIR}/.venv/bin/pip" install --prefer-binary --extra-index-url "${fi_index}" "${fi_pkg}"; then
      log_warn "FlashInfer install failed with extra index; falling back to PyPI only"
    ...
    log_info "FlashInfer wheel source: ${fi_index} (CUDA=${cuda_ver} Torch=${torch_ver})"
  else
...
vllm_validate_installation() {
  ...
  if ! vllm_is_installed "${python_exec}"; then
    log_err "vLLM not installed or not importable"
    return 1
  fi
  
  local vllm_ver
  vllm_ver="$(vllm_get_version "${python_exec}")"
  log_info "vLLM version: ${vllm_ver}"
  
  if vllm_has_flashinfer "${python_exec}"; then
    log_info "FlashInfer: available"
    export HAS_FLASHINFER=1
  else
    log_info "FlashInfer: not available (using XFORMERS)"
