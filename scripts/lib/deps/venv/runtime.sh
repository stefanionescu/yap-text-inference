...
# =============================================================================
# Sourced by scripts/lib/deps/venv/main.sh after common dependencies are loaded.

_venv_install_python310_deadsnakes() {
  log_info "[python] Installing Python ${CFG_TRT_REQUIRED_PYTHON_VERSION} via deadsnakes PPA..."

  if ! command -v apt-get >/dev/null 2>&1; then
    log_err "[python] ✗ apt-get not available. Python ${CFG_TRT_REQUIRED_PYTHON_VERSION} installation requires Ubuntu/Debian."
    log_err "[python] ✗ Please install Python ${CFG_TRT_REQUIRED_PYTHON_VERSION} manually."
    return 1
  fi
  ...
  DEBIAN_FRONTEND=noninteractive ${run_apt} install -y --no-install-recommends \
    ...
    log_err "[python] ✗ Failed to install Python ${CFG_TRT_REQUIRED_PYTHON_VERSION}"
    return 1
  }

  log_info "[python] ✓ Python ${CFG_TRT_REQUIRED_PYTHON_VERSION} installed successfully"
}

_venv_python_reports_version() {
  local py_bin="$1"
  local expected_minor="${2:-${CFG_TRT_REQUIRED_PYTHON_VERSION}}"
...
ensure_python_runtime_for_engine() {
  ...
  if [[ ${engine,,} == "trt" ]]; then
    if _venv_python_reports_version "python${CFG_TRT_REQUIRED_PYTHON_VERSION}"; then
      return 0
    fi

    log_warn "[python] ⚠ Python ${CFG_TRT_REQUIRED_PYTHON_VERSION} not found. TRT-LLM requires Python ${CFG_TRT_REQUIRED_PYTHON_VERSION}."
    log_info "[python] Attempting to install Python ${CFG_TRT_REQUIRED_PYTHON_VERSION}..."

    if _venv_install_python310_deadsnakes; then
      return 0
    fi

    log_err "[python] ✗ Cannot proceed without Python ${CFG_TRT_REQUIRED_PYTHON_VERSION}"
    ...
    log_err "[python]     Or use the Docker image which has Python ${CFG_TRT_REQUIRED_PYTHON_VERSION} pre-installed"
    return 1
  fi
...
get_python_binary_for_engine() {
  ...
  if [ "${engine}" = "trt" ] || [ "${engine}" = "TRT" ]; then
    ...
    if command -v "python${CFG_TRT_REQUIRED_PYTHON_VERSION}" >/dev/null 2>&1; then
      echo "python${CFG_TRT_REQUIRED_PYTHON_VERSION}"
    elif command -v python3.10 >/dev/null 2>&1; then
      echo "python3.10"
    else
      log_err "[venv] ✗ Python ${CFG_TRT_REQUIRED_PYTHON_VERSION} required for TRT-LLM but not found"
...
validate_venv_python_version() {
  ...
  if [ "${engine}" = "trt" ] || [ "${engine}" = "TRT" ]; then
    if [ "${current_version}" != "${CFG_TRT_REQUIRED_PYTHON_VERSION}" ]; then
      log_warn "[venv] ⚠ TRT requires Python ${CFG_TRT_REQUIRED_PYTHON_VERSION} but venv has ${current_version}"
      return 1
    fi
  fi
